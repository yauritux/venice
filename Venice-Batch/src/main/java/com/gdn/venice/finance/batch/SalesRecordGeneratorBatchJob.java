    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String query = "select o from VenOrderItem o "
                    + " where o.salesBatchStatus in ('Ready', 'Failed') ";
            _log.info("Query: " + query);
            List<VenOrderItem> items = itemHome.queryByRange(query, 0, 100);
            ...
            for (VenOrderItem item : items) {
                try {
                    item.setSalesBatchStatus("In Process");
            ...
            for (VenOrderItem item : items) {
                try {
                    salesRecord = null;
                    if (item.getSalesBatchStatus().equals("In Process")) {
                        ...
                        if (salesRecord != null) {
                            ...
                            item.setSalesBatchStatus("Ready Journal");
                            count++;
                        } else {
