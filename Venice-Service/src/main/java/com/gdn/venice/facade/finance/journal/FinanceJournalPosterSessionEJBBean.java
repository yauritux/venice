...
public class FinanceJournalPosterSessionEJBBean implements
            ...
            for (FinArFundsInReconRecord reconRecord : reconRecordList) {
                ...
                if (!finArFundsInJournalTransactionList.isEmpty() && itemsAllocate != null && reconRecord.getFinArFundsInActionApplied().getActionAppliedId().equals(VeniceConstants.FIN_AR_FUNDS_IN_ACTION_APPLIED_ALLOCATED)) {
                    ...
                    postAllocationJournal(
                            reconRecord, fundsInReconRecordHome, journalApprovalGroupHome, journalTransactionHome, finArFundsInAllocateHome, itemsAllocate.get(0), reconRecordList);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public Boolean postAllocationJournal(FinArFundsInReconRecord reconRecord, FinArFundsInReconRecordSessionEJBLocal fundsInReconRecordHome, FinJournalApprovalGroupSessionEJBLocal journalApprovalGroupHome, FinJournalTransactionSessionEJBLocal journalTransactionHome, FinArFundsInAllocatePaymentSessionEJBLocal finArFundsInAllocateHome, FinArFundsInAllocatePayment finArFundsInAllocatePayment, List<FinArFundsInReconRecord> reconRecordList) {
...
//					}
    ...
    public Boolean postSalesJournalTransaction(Long finSalesRecordId, Boolean pkp) {
        ...
        try {
            ...
            List<FinSalesRecord> finSalesRecordList = salesRecordHome.queryByRange("select o from FinSalesRecord o inner join fetch o.venOrderItem i left join fetch i.venOrderItemAdjustments where o.salesRecordId = " + finSalesRecordId, 0, 0);
            if (finSalesRecordList.isEmpty()) {
...
//			}
	 ...
	 * @see
	 * com.gdn.venice.facade.finance.journal.FinanceJournalPosterSessionEJBRemote
	 * #postPaymentRefundJournalTransaction(FinArFundsInRefund finArFundsInRefund)
	 */
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public Boolean postPaymentRefundJournalTransaction(
		...
		try {
			...
			for (FinArFundsInRefund finArFundsInRefund:finArFundsInRefundList){
					...
					long accountNumberBank = 0;
					if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_BCA_CC
							|| finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_BCA_IB) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120104;
					} else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_BCA_VA) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120102;
					}else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB ||
							finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC ||
							finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC) {					
							accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120105;
					} else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_MANDIRI_VA) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120301;
					} else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_MANDIRI_IB ||
							finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_MANDIRIINSTALLMENT_CC ) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120302;
					} else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_NIAGA_IB) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120402;
					}else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_XL_IB) {
						accountNumberBank = VeniceConstants.FIN_ACCOUNT_1120888;
					}else if (finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeId() == VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_BRI_IB) {
							accountNumberBank = VeniceConstants.FIN_ACCOUNT_1121001;
					}else{
						throw new EJBException("Account number not available for the payment, please add account number to fin account and venice constants");
					}			
					...
					if(finArFundsInRefund.getRefundType().contains("Bank")){				
						...
						amountBiaya= finArFundsInRefund.getFinArFundsInReconRecord().getProviderReportFeeAmount()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getProviderReportFeeAmount():new BigDecimal(0);		
					}else{				
					...
					refundBankJournalTransaction= setJournalTransaction(finArFundsInRefund.getFinArFundsInReconRecord(),(finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId():""), finJournalApprovalGroup, 
							accountIdRefund, VeniceConstants.FIN_JOURNAL_PAYMENT, VeniceConstants.FIN_TRANSACTION_STATUS_NEW, finTransactionTypeID, time,  
							finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment().getVenBank():finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getVenBank(),
							 journalTransactionHome,accountNumberBank,finArFundsInRefund.getApAmount(),false,"");
					...
					biayaJournalTransaction= setJournalTransaction(finArFundsInRefund.getFinArFundsInReconRecord(),(finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId():""), finJournalApprovalGroup, 
							accountIdForBiaya, VeniceConstants.FIN_JOURNAL_PAYMENT, VeniceConstants.FIN_TRANSACTION_STATUS_NEW, finTransactionTypeID, time,  
							finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment().getVenBank():finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getVenBank(),
							 journalTransactionHome,accountNumberBank,amountBiaya,false,"");
					...
					bankJournalTransaction= setJournalTransaction(finArFundsInRefund.getFinArFundsInReconRecord(),(finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getWcsOrderId():""), finJournalApprovalGroup, 
							accountNumberBank, VeniceConstants.FIN_JOURNAL_PAYMENT, VeniceConstants.FIN_TRANSACTION_STATUS_NEW, finTransactionTypeID, time,  
							finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment()!=null?finArFundsInRefund.getFinArFundsInReconRecord().getVenOrderPayment().getVenBank():finArFundsInRefund.getFinArFundsInReconRecord().getFinArFundsInReport().getFinArFundsInReportType().getVenBank(),
							 journalTransactionHome,accountNumberBank,amountBiaya.add(finArFundsInRefund.getApAmount()),true,"");
					_log.debug("Save ( Bank Transaks)::END");								
					transactionList.add(bankJournalTransaction);		
			
			}
			
			FinApPayment finApPayment=finArFundsInRefundList.get(0).getFinApPayment();
			finApPayment.setFinJournalTransactions(transactionList);		
			//Set status to Done
			FinApprovalStatus approvalStatus = new FinApprovalStatus();
			approvalStatus.setApprovalStatusId(new Long(4));
		...
		Long endTime = System.currentTimeMillis();
		Long duration = endTime - startTime;
		_log.debug("postPaymentRefundJournalTransaction()" + " completed in " + duration + "ms");
		return true;
	}
}
