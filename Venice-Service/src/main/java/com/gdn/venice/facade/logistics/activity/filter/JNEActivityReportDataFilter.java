...
@Service("JNEActivityReportDataFilter")
public class JNEActivityReportDataFilter extends ActivityReportDataFilter {
	protected static Logger _log = Log4jLoggerFactory.getLogger("com.gdn.venice.facade.logistics.activity.filter.JNEActivityReportDataFilter");
	...
	public ActivityReportData filter(ActivityReportData activityReportDataToBeFiltered) throws ActivityReportDataFilterException{
		...
		for(int i = 0; i < totalItemBeforeFilter; i++){
			...
			dailyReportOrderItem = (DailyReportJNE) activityReportDataToBeFiltered.getOrderItemList().get(i);
			
			if(isItemOk(dailyReportOrderItem.getGdnRefNumber(), activityReportFilterResult)){
