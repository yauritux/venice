        ...
        try {
            ...
            if (request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) != "") {
                ...
                if (request.getParams().get(DataNameTokens.VENORDER_ORDERID).equals(VeniceConstants.FRD_ORDER_HISTORY_CUSTOMER_ADDRESS_FILTER)) {
                    if (!request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).equals("")) {
                        query = "select o from VenOrder o left join fetch o.venOrderContactDetails where o.wcsOrderId <> '" 
                                + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and ( o.venCustomer.venParty.partyId in "
                                + "(select u.venParty.partyId from VenPartyAddress u where UPPER(u.venAddress.streetAddress1) in ('" + request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).toUpperCase().replace("'", "''") + "')) or "
                                + "o.orderId in (select u.venOrder.orderId from VenOrderAddress u where UPPER(u.venAddress.streetAddress1) in ('" + request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).toUpperCase().replace("'", "''") + "')) )";
                        itemList = ordersessionHome.queryByRange(query, request.getStartRow(), request.getEndRow() - request.getStartRow());
                        count = ordersessionHome.queryByRange(query, 0, 0);
                    }
                    ...
                    if (!request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).equals("")) {
                        query = "select o from VenOrder o left join fetch o.venOrderContactDetails where o.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and o.orderId in "
                                + "(select u.venOrder.orderId from VenOrderItem u where UPPER(u.venAddress.streetAddress1) like '" + request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).toUpperCase().replace("'", "''") 
                                + "' group by  u.venOrder.orderId)";
                        itemList = ordersessionHome.queryByRange(query, request.getStartRow(), request.getEndRow() - request.getStartRow());
                        count = ordersessionHome.queryByRange(query, 0, 0);
                    }
                    ...
                    query = "select o from VenOrder o left join fetch o.venOrderContactDetails where o.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and o.orderId in "
                            + "(select u.venOrder.orderId from VenOrderPaymentAllocation u where UPPER(u.venOrderPayment.venAddress.streetAddress1) like '" + request.getParams().get(DataNameTokens.ORDERHISTORY_STRINGFILTER).toUpperCase().replace("'", "''") + "')";
                    itemList = ordersessionHome.queryByRange(query, request.getStartRow(), request.getEndRow() - request.getStartRow());
                    count = ordersessionHome.queryByRange(query, 0, 0);
