	...
	public RafDsResponse execute() {
		...
		try {
			...
			for (int i=0;i<finSalesRecordList.size();i++) {
				...
				map.put(DataNameTokens.FINSALESRECORD_VENORDERITEM_VENSETTLEMENTRECORDS_PPH23_FLAG, salesRecord.getVenOrderItem().getVenSettlementRecords()!=null? (salesRecord.getVenOrderItem().getVenSettlementRecords().get(0).getPph23()?"Yes":"No"):"");
				...
				//get payment data
				String paymentQuery = "select o from VenOrderPaymentAllocation o join fetch o.venOrderPayment oi join fetch oi.finArFundsInReconRecords oe where o.venOrder.orderId = " + salesRecord.getVenOrderItem().getVenOrder().getOrderId();
				
				paymentList = paymentSessionHome.queryByRange(paymentQuery, 0, 0);
				if(paymentList.size()>0){
					map.put(DataNameTokens.FINSALESRECORD_VENORDERITEM_VENORDER_VENORDERPAYMENTALLOCATION_VENORDERPAYMENT_VENWCSPAYMENTTYPE_PAYMENTTYPECODE, paymentList.get(0).getVenOrderPayment().getVenWcsPaymentType().getWcsPaymentTypeCode());
					for(VenOrderPaymentAllocation venOrderPaymentAllocationItem : paymentList){
						if(venOrderPaymentAllocationItem.getVenOrderPayment().getFinArFundsInReconRecords()!=null && venOrderPaymentAllocationItem.getVenOrderPayment().getFinArFundsInReconRecords().get(0).getFinArFundsInReport() !=null){
							map.put(DataNameTokens.FINSALESRECORD_VENORDERITEM_VENORDER_VENORDERPAYMENTALLOCATIONS_VENORDERPAYMENT_FINARFUNDSINRECONRECORDS_FINARFUNDSINREPORT_FINARFUNDSINREPORTTYPE_PAYMENTREPORTTYPEDESC,venOrderPaymentAllocationItem.getVenOrderPayment().getFinArFundsInReconRecords().get(0).getFinArFundsInReport().getFinArFundsInReportType().getPaymentReportTypeDesc());
							break;
						}						
					}
				}
