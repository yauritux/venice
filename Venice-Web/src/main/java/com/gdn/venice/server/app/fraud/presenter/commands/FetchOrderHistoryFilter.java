    ...
    public RafDsResponse execute() {
        ...
        try {
            ...
            if (request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) != "") {
                 ...
                 * get data order item
                 */
                query = "select oi from VenOrderItem as oi join fetch oi.venOrder as o join fetch o.venOrderContactDetails as ocd  where o.wcsOrderId='" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "'";
                        ...
                        if (i == 1) {
                            ...
                            if (itemList.get(0).getVenOrder().getVenCustomer().getVenParty() != null) {
                                if (!itemList.get(0).getVenOrder().getVenCustomer().getCustomerUserName().toUpperCase().equals("ANONYMOUS") && itemList.get(0).getVenOrder().getVenCustomer().getVenParty() != null) {
                                    query = "select o from VenOrder o where o.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and UPPER(o.venCustomer.venParty.fullOrLegalName) like '" + itemList.get(0).getVenOrder().getVenCustomer().getVenParty().getFullOrLegalName().toUpperCase().replace("'", "''") + "'";
                            ...
                            if (address != null && !address.isEmpty() && address.size() > 0) {                                
                                ...
                                query = "select o from VenOrder o where o.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) 
                                        + "' and ( o.venCustomer.venParty.partyId in "
                                        + "(select u.venParty.partyId from VenPartyAddress u where UPPER(u.venAddress.streetAddress1) in ('" + addressItem.replace("'", "''") + "')) or "
                                        + "o.orderId in (select u.venOrder.orderId from VenOrderAddress u where UPPER(u.venAddress.streetAddress1) in ('" + addressItem.replace("'", "''") + "'))) ";
                                List<VenOrder> orderList = orderSessionHome.queryByRange(query, 0, 0);
                                count = orderList.isEmpty() ? 0 : orderList.size();
                            ...
                            if (itemList.get(0).getVenAddress().getStreetAddress1() != null) {
                                query = "select u from VenOrder u where u.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and u.orderId in "
                                        + "(select o.venOrder.orderId from VenOrderItem o where UPPER(o.venAddress.streetAddress1) like '" + itemList.get(0).getVenAddress().getStreetAddress1().toUpperCase().replace("'", "''") 
                                        + "' group by o.venOrder.orderId)";
                            ...
                            if (allocationList != null && !allocationList.isEmpty()) {
                                query = "select o from VenOrder o where o.wcsOrderId <> '" + request.getParams().get(DataNameTokens.VENORDER_WCSORDERID) + "' and o.orderId in "
                                        + "(select u.venOrder.orderId from VenOrderPaymentAllocation u where UPPER(u.venOrderPayment.venAddress.streetAddress1) like '" + allocationList.get(0).getVenOrderPayment().getVenAddress().getStreetAddress1().toUpperCase().replace("'", "''") + "')";
                                List<VenOrder> orderList = orderSessionHome.queryByRange(query, 0, 0);
                                count = orderList.isEmpty() ? 0 : orderList.size();
                            }                            
