...
public class MakePaymentCommand implements RafRpcCommand{
	...
	public String execute() {
		...
		if (paymentTo.equals("MERCHANT")) {
			...
			Double penaltyAmount = new Double(0);
			Long bankAccountId = new Long(0);
			Double feeAmount = new Double(0);
			ArrayList<Long> finArFundsInRefundId=null;
			Locator<Object> locator=null;
			try {				
				
				locator=new Locator<Object>();
				
				FinancePaymentCreatorSessionEJBRemote sessionHome = (FinancePaymentCreatorSessionEJBRemote) locator
						.lookup(FinancePaymentCreatorSessionEJBRemote.class,
								"FinancePaymentCreatorSessionEJBBean");
				
				FinArFundsInRefundSessionEJBRemote refundSessionHome = (FinArFundsInRefundSessionEJBRemote) locator
						.lookup(FinArFundsInRefundSessionEJBRemote.class,
								"FinArFundsInRefundSessionEJBBean");
				
				ArrayList<Long> manualJournalTransactionList = new ArrayList<Long>();
				finArFundsInRefundId=new ArrayList<Long>();
				for (int i=0;i<paymentDataMap.size();i++) {
						String logisticsPaymentString = paymentDataMap.get("PAYMENTDATA" + i);
						...
						finArFundsInRefundId.add(new Long(refundRecordIdParam));			
						...
						if (finApManualJournalIdParam!=null) {
						 ...
						 paymentAmount =paymentAmount + new Double(refundPaymentMap.get("AMOUNT"));
						 penaltyAmount = penaltyAmount + new Double(refundPaymentMap.get("PENALTYAMOUNT"));
						 bankAccountId =new Long(refundPaymentMap.get("BANKACCOUNTID"));
						 if(refundPaymentMap.get("FEEAMOUNT")!=null){
								 feeAmount=feeAmount + new Double(refundPaymentMap.get("FEEAMOUNT"));
								 
								 List<FinArFundsInRefund> finArFundsInRefundList  =refundSessionHome.queryByRange("select o from FinArFundsInRefund o where o.refundRecordId="+refundRecordIdParam, 0, 0);
								 if(!finArFundsInRefundList.isEmpty()){
									 FinArFundsInRefund item = finArFundsInRefundList.get(0);
									 item.setFeeAmount(new BigDecimal(refundPaymentMap.get("FEEAMOUNT")));
									 item=refundSessionHome.mergeFinArFundsInRefund(item);
								 }
						 }
