...
//		fraudCaseManagementListGrid.setAutoFetchData(true);	
	...
	private void showFraudCaseDetails(Record record) {
		...
		String wcsOrderId =  record.getAttributeAsString(DataNameTokens.FRDFRAUDSUSPICIONCASE_VENORDER_WCSORDERID);
		List<DataSource> dataSources = getUiHandlers().onShowFraudCaseDetailData(taskId, caseId, wcsOrderId);
		...
		fraudCaseManagementContentLayout = new FraudCaseManagementLayout(
				...
				dataSources.get(25),//order history data
				dataSources.get(26)//venmigsupload
				);
