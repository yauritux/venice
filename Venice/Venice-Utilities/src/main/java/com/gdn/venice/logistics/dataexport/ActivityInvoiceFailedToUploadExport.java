    ...
    public HSSFWorkbook ExportExcel(ArrayList<String> failedMessage, HSSFSheet sheet) throws ServletException {
        ...
        try {
            ...
            for (int i = 0; i < failedMessage.size(); i++) {
                ...
                nameCell.setCellValue(new HSSFRichTextString(failedMessage.get(i)));
    ...
    public HSSFWorkbook ExportExcel(HashMap<String, String> gdnRefNotFoundList, HashMap<String, String> failedRecord, List<FailedStatusUpdate> failedStatusUpdateList, HSSFSheet sheet, String source) throws Exception {
        ...
        if (!source.equals("invoice")) {
            ...
            while (gdnRefIterator.hasNext()) {
                ...
                HSSFRow row = sheet.createRow(startRow);
                HSSFCell gdnReffCell = row.createCell(startCol);
                gdnReffCell.setCellValue(new HSSFRichTextString(gdnRefMap.getKey().toString()));
                HSSFCell problemCell = row.createCell(startCol + 1);
                problemCell.setCellValue(new HSSFRichTextString(gdnRefMap.getValue().toString()));
        ...
        if (!source.equals("invoice")) {
            ...
            headerStatusUpdateProblem.createCell(startCol + 5).setCellValue(new HSSFRichTextString("Used on item(s)"));
            ...
            for (FailedStatusUpdate failedStatusUpdate : failedStatusUpdateList) {
                ...
                HSSFRow row = sheet.createRow(startRow);
                HSSFCell gdnRefCell = row.createCell(startCol);
                gdnRefCell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getGdnRef()));
                HSSFCell awbLogisticCell = row.createCell(startCol + 1);
                awbLogisticCell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getAirwayBillNoLogistic()));
                HSSFCell awbMTACell = row.createCell(startCol + 2);
                awbMTACell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getAirwayBillNoMTA()));
                HSSFCell orderItemStatusCell = row.createCell(startCol + 3);
                orderItemStatusCell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getOrderItemStatus()));
                HSSFCell awbStatusCell = row.createCell(startCol + 4);
                awbStatusCell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getAirwayBillStatus()));
                HSSFCell usedOnItemCell = row.createCell(startCol + 5);
                usedOnItemCell.setCellValue(new HSSFRichTextString(failedStatusUpdate.getUsedOnItems()));
