...
public class UpdateOrderFPBatchJob {
    ...
    private Boolean findAndUpdateOrderStatusToFP() {
        ...
        try {
            ...
            String selectOrderPaymentAllocation = "select o from VenOrderPaymentAllocation o join fetch o.venOrder ord join fetch ord.venOrderItems"
                    ...
                    for (int k = 0; k < orderPaymentAllocationList.size(); k++) {
                        ...
                        if (orderPaymentAllocationList.get(k).getVenOrder().getWcsOrderId().equals(wcsOrderIdList.get(j))) {
                            ...
                            if (!creditCardNumber.equals("") || creditCardNumber != null) {
	                            ...
	                            String query="select o from VenBinCreditLimitEstimate o where o.bankName='BNI 46' and upper(o.description) like upper('%Debit%') and o.binNumber like '"+ binNumber +"' ";
	                            binCreditLimitList=binCreditLimitSessionHome.queryByRange(query, 0, 0);
                            }
                            if(binCreditLimitList!=null && !binCreditLimitList.isEmpty() ){	
                            	    _log.info("BNI Debit BIN number registered, update FP automatically, wcsOrderId: " + orderPaymentAllocationList.get(tempCounter).getVenOrder().getWcsOrderId());
                            	
                            }
                            else if (orderPaymentAllocationList.get(k).getVenOrderPayment().getVenPaymentType().getPaymentTypeCode().equals(VeniceConstants.VEN_PAYMENT_TYPE_CC)) {
