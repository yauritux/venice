...
public class FinancePaymentCreatorSessionEJBBean implements FinancePaymentCreatorSessionEJBRemote, FinancePaymentCreatorSessionEJBLocal {
	...
	public Boolean createRefundPayment(
			ArrayList<Long> finArFundsInRefundId,
			ArrayList<Long> manualJournalTransactionList,
			Double penaltyAmount,
			Double paymentAmount,
			Long bankAccountId) {
		...
		try {
			...
			String IdRefund="";
			for(int i=0;i<finArFundsInRefundId.size();i++){
				if(IdRefund.equals("")) IdRefund=finArFundsInRefundId.get(i)+"";
				else IdRefund=IdRefund+","+finArFundsInRefundId.get(i);
			}
			
			List<FinArFundsInRefund> finArFundsInRefundList = refundRecordHome.queryByRange("select o from FinArFundsInRefund o where o.refundRecordId in (" + IdRefund +")" , 0, 0);
			...
			for(FinArFundsInRefund item : finArFundsInRefundList){
				item.setFinApPayment(payment);
				refundRecordHome.mergeFinArFundsInRefund(item);
			}		
