...
public class AWBReconciliation {
    ...
    private static final String ACTIVITY_RECON_RECORD_FROM_CX_MTA_SELECT_BY_AIRWAYBILLID_SQL = "select * "
            + "from log_activity_recon_record arr "
            + "inner join log_recon_activity_record_result rarr on arr.recon_record_result_id = rarr.recon_record_result_id "
            + "where airway_bill_id = ? "
            + "and arr.recon_record_result_id <> 2";
    ...
    public LogAirwayBill performActivityReconciliation(Connection conn, LogActivityReportUpload logActivityReportUpload, LogAirwayBill providerAirwayBill, LogAirwayBill mtaAirwayBill,
            String wcsOrderItemId, boolean isMcx)  {
...
//						.queryByRange("select o from LogActivityReconRecord o where o.logAirwayBill.airwayBillId =" + mtaAirwayBill.getAirwayBillId(), 0, 0);
                ...
                PreparedStatement psActivityReconRecord;
                if (isMcx) {
                    psActivityReconRecord = conn.prepareStatement(ACTIVITY_RECON_RECORD_FROM_CX_MTA_SELECT_BY_AIRWAYBILLID_SQL);
                } else {
                    psActivityReconRecord = conn.prepareStatement(ACTIVITY_RECON_RECORD_SELECT_BY_AIRWAYBILLID_SQL);
                }
...
//				}
            ...
            if (mtaAirwayBill.getActualPickupDate() != null && 
            	providerAirwayBill.getActualPickupDate() != null && 
            	providerAirwayBill.getActualPickupDate().compareTo(mtaAirwayBill.getActualPickupDate()) != 0) {
...
//				}
            ...
            if (mtaAirwayBill.getVenOrderItem() != null && !providerServiceCode.equalsIgnoreCase(mtaServiceCode)) {
                ...
                logActivityReconRecord.setLogReconActivityRecordResult(logReconActivityRecordResult);
                logActivityReconRecord.setProviderData(providerServiceCode);
                logActivityReconRecord.setVeniceData(mtaServiceCode);
...
//			}
            ...
            if (rsActivityReconRecord.next()) {
                providerAirwayBill.setActivityResultStatus("Problem Exists");
            } else {
                providerAirwayBill.setActivityResultStatus("OK");
            }
...
//			}
    ...
    public LogAirwayBill performActivityReconciliation(Connection conn, LogActivityReconRecordDAO logActivityReconRecordDAO, LogActivityReportUpload logActivityReportUpload, LogAirwayBill providerAirwayBill, LogAirwayBill mtaAirwayBill,
            String wcsOrderItemId, boolean isMcx)  {
        Locator<Object> locator = null;
        _log.info("start performActivityReconciliation");
        try {
...
//						.queryByRange("select o from LogActivityReconRecord o where o.logAirwayBill.airwayBillId =" + mtaAirwayBill.getAirwayBillId(), 0, 0);
            ...
            String serviceCodeFromMTA = "";
            String recipientFromMTA = "";
            String airwaybillFromMTA = "";
            Date actualPickupDateFromMTA = null;
            
            String serviceCodeFromLogistic = "";
            String recipientFromLogistic = "";
            String airwaybillFromLogistic = "";
            Date actualPickupDateFromLogistic = null;
            ...
            if (providerAirwayBill.getLogActivityReconRecords() == null) {
                ...
                if (logActivityReconRecordList != null && logActivityReconRecordList.size() > 0) {
                    for (LogActivityReconRecord logActivityReconRecord : logActivityReconRecordList) {
                        ...
                        if (logReconActivityRecordResult.getReconRecordResultId() == VeniceConstants.LOG_ACTIVITY_RECON_RESULT_PICKUP_DATE_MISMATCH) {
                        	actualPickupDateFromMTA = new SimpleDateFormat("dd-MM-yyyy").parse(logActivityReconRecord.getVeniceData());
//                            mtaAirwayBill.setActualPickupDate(new SimpleDateFormat("dd-MM-yyyy").parse(logActivityReconRecord.getVeniceData()));
                        }
...
//								mtaAirwayBill.setAirwayBillNumber(logActivityReconRecord.getVeniceData());
//							} else if (providerAirwayBill.getLogLogisticsProvider().getLogisticsProviderCode().equalsIgnoreCase("JNE")) {
                            airwaybillFromMTA = logActivityReconRecord.getVeniceData();
//                        	mtaAirwayBill.setAirwayBillNumber(logActivityReconRecord.getVeniceData());
//							}
                        }

                        if (logReconActivityRecordResult.getReconRecordResultId() == VeniceConstants.LOG_ACTIVITY_RECON_RESULT_SERVICE_MISMATCH) {
                        	serviceCodeFromMTA = logActivityReconRecord.getVeniceData();
//                            mtaAirwayBill.getVenOrderItem().getLogLogisticService().setServiceCode(logActivityReconRecord.getVeniceData());
                        }

                        if (logReconActivityRecordResult.getReconRecordResultId() == VeniceConstants.LOG_ACTIVITY_RECON_RESULT_RECIPIENT_MISMATCH) {
                        	recipientFromMTA = logActivityReconRecord.getVeniceData();
//                            mtaAirwayBill.getVenOrderItem().getVenRecipient().getVenParty().setFullOrLegalName(logActivityReconRecord.getVeniceData());
                        }
                ...
                if (logActivityReconRecordList != null && logActivityReconRecordList.size() > 0) {
                    for (LogActivityReconRecord logActivityReconRecord : logActivityReconRecordList) {
                        ...
                        if (logReconActivityRecordResult.getReconRecordResultId() == VeniceConstants.LOG_ACTIVITY_RECON_RESULT_PICKUP_DATE_MISMATCH) {
                        	actualPickupDateFromLogistic = new SimpleDateFormat("dd-MM-yyyy").parse(logActivityReconRecord.getProviderData());
//                            providerAirwayBill.setActualPickupDate(new SimpleDateFormat("dd-MM-yyyy").parse(logActivityReconRecord.getProviderData()));
                        }
...
//								providerAirwayBill.setAirwayBillNumber(logActivityReconRecord.getProviderData());
//							} else if (providerAirwayBill.getLogLogisticsProvider().getLogisticsProviderCode().equalsIgnoreCase("JNE")) {
                        	airwaybillFromLogistic = logActivityReconRecord.getProviderData();
//                            providerAirwayBill.setAirwayBillNumber(logActivityReconRecord.getProviderData());
//							}
                        }
...
//                        	providerAirwayBill.setService(logActivityReconRecord.getProviderData());
                        ...
                        if (logReconActivityRecordResult.getReconRecordResultId() == VeniceConstants.LOG_ACTIVITY_RECON_RESULT_RECIPIENT_MISMATCH) {
                            recipientFromLogistic = logActivityReconRecord.getProviderData();
//                        	providerAirwayBill.setConsignee(logActivityReconRecord.getProviderData());
                        }
                    }
                }
            ...
            Boolean problemExists = false;
            ArrayList<LogActivityReconRecord> activityReconRecordList = new ArrayList<LogActivityReconRecord>(5);
            ...
            //  01 - Pickup date is late o According to spec - This needs to be fished from the SLA and compared 
            // o The spec is WRONG because we have actual from MTA to compare the date with. Late pickup is handled by SLA and KPI. This routine only needs to identify discrepancies.			 
            if (actualPickupDateFromMTA != null && actualPickupDateFromLogistic.compareTo(actualPickupDateFromMTA) != 0) {
                _log.debug("Pickup date mismatch found for AWB activity:" + mtaAirwayBill.getAirwayBillNumber());
...
//                queryBuilder.append("select o from LogAirwayBill o where o.airwayBillId = ");
                ...
                // means that new data comes from Logistic / Provider
                if (providerAirwayBill.getLogActivityReconRecords() == null) {
                	mtaAirwayBill.setActualPickupDate(providerAirwayBill.getActualPickupDate());
//                    queryBuilder.append(mtaAirwayBill.getAirwayBillId());
                    // means that new data comes from MTA
                } else if (mtaAirwayBill.getLogActivityReconRecords() == null) {
                	providerAirwayBill.setActualPickupDate(providerAirwayBill.getActualPickupDate());
//                    queryBuilder.append(providerAirwayBill.getAirwayBillId());
                }
...
//                mtaAirwayBill.getVenOrderItem().getLogLogisticService().setServiceCode("MSG");
            ...
            if (mtaAirwayBill.getVenOrderItem() != null && !serviceCodeFromLogistic.equalsIgnoreCase(serviceCodeFromMTA)) {
                ...
                logActivityReconRecord.setLogReconActivityRecordResult(logReconActivityRecordResult);
                logActivityReconRecord.setProviderData(serviceCodeFromLogistic);
                logActivityReconRecord.setVeniceData(serviceCodeFromMTA);

                activityReconRecordList.add(logActivityReconRecord);
                problemExists = true;
            }

            //04 - Recipient mismatch (compare consignee from report with recipient party full or legal name)			 
            if (mtaAirwayBill.getVenOrderItem() != null && !(recipientFromLogistic != null ? recipientFromLogistic.trim() : "").equalsIgnoreCase(recipientFromMTA)) {
                _log.debug("Recipient mismatch found for AWB activity:" + mtaAirwayBill.getAirwayBillNumber());
                LogActivityReconRecord logActivityReconRecord = new LogActivityReconRecord();
                ...
                logActivityReconRecord.setLogReconActivityRecordResult(logReconActivityRecordResult);
                logActivityReconRecord.setProviderData(recipientFromLogistic != null ? recipientFromLogistic.trim() : "");
                logActivityReconRecord.setVeniceData(recipientFromMTA);
            ...
            if (activityReconRecordList.size() > 0) {
                providerAirwayBill.setActivityResultStatus("Problem Exists");
                logActivityReconRecordDAO.save(activityReconRecordList);
            } else {
                providerAirwayBill.setActivityResultStatus("OK");
            }
...
//			}
     ...
     * @throws ServletException
     */
    public void performInvoiceReconciliation(LogInvoiceReportUpload logInvoiceReportUpload, LogAirwayBill providerAirwayBill, AirwayBillTransactionResource mtaAirwayBill, HashMap<String, String> failedRecord) {
