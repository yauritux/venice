...
import com.gdn.venice.facade.logistics.activity.parser.NCSActivityReportFileParser;
...
@Service("NCSActivityReportProcessor")
public class NCSActivityReportProcessor extends ActivityReportProcessor {

	protected static Logger _log = Log4jLoggerFactory.getLogger("com.gdn.venice.facade.logistics.activity.processor.NCSActivityReportProcessor");
	
	@PersistenceContext
	EntityManager em;
	
	@Autowired
    @Qualifier("NCSActivityReportDataFilter")
	...
	ActivityReportFileParser fileParser = new NCSActivityReportFileParser();
	...
	public void process(LogFileUploadLog fileUploadLog) {
		try {
		    ...
		    for(PojoInterface item : activityReportData.getOrderItemList()){
		    	dailyReportOrderItem = (DailyReportNCS) item;
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public void processEachReturnItem(DailyReportNCS dailyReportOrderItem){
...
//		}
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public void processEachOrderItem(DailyReportNCS dailyReportOrderItem, ActivityReportData activityReportData, LogFileUploadLog fileUploadLog){
	...
	public void processOrderItemAfterAWBEngine(VenOrderItem existingVenOrderItem,
										       DailyReportNCS dailyReportOrderItem, 
	...
	public boolean composeLogAirwayBill(LogAirwayBill logAirwayBill, DailyReportNCS dailyReportOrderItem, ActivityReportData activityReportData, LogFileUploadLog fileUploadLog){
	...
	public boolean differentLogisticProvider(DailyReportNCS dailyReportOrderItem, ActivityReportData activityReportData, String kodeLogisticPorvider){	
		if(!kodeLogisticPorvider.equalsIgnoreCase("NCS")){
	...
	private String getRecipient(DailyReportNCS dailyReportOrderItem){
	...
	private boolean isOrderItemEligibleForStatusUpdate(DailyReportNCS dailyReportOrderItem, long existingOrderItemStatusId){
	...
	private VenOrderStatusConstants getOrderStatusChange(DailyReportNCS dailyReportOrderItem){
	...
	private String getAWBStatusChange(DailyReportNCS dailyReportOrderItem){
		if (dailyReportOrderItem.getStatus().equalsIgnoreCase("OK")
             && (!dailyReportOrderItem.getReceived().isEmpty() && dailyReportOrderItem.getReceived() != null)
             && (!dailyReportOrderItem.getRecipient().isEmpty() && dailyReportOrderItem.getRecipient() != null)) {
	...
	private void addFailedRecordCausedByAWBException(DailyReportNCS dailyReportNCS, ActivityReportData activityReportData, Exception e){
	...
	private void addFailedRecordCausedByReceivedDateException(DailyReportNCS dailyReportNCS, ActivityReportData activityReportData, Exception e){
		...
		activityReportData.getFailedItemList()
			.put("No : " + dailyReportNCS.getTrNo().replace(".0", "") + ", GDN Ref : " + dailyReportNCS.getRefNo(), "Ops please fix Received Date : " + dailyReportNCS.getReceived());
	}

	private void addFailedRecordCausedByPickupTimeException(DailyReportNCS dailyReportNCS, ActivityReportData activityReportData, Exception e){
		_log.error("CNoteDate problem", e);
        activityReportData.getFailedItemList()
        	.put("No : " + dailyReportNCS.getTrNo().replace(".0", "") + ", GDN Ref : " + dailyReportNCS.getRefNo(), "Ops please fix PuDate : " + dailyReportNCS.getPuDate());
            
	}
	
	private void addFailedRecordCausedByAWBReconciliationException(DailyReportNCS dailyReportNCS, ActivityReportData activityReportData, Exception e){
		_log.error("AWBReconciliation Problem", e);
		activityReportData.getFailedItemList()
			.put("No : " + dailyReportNCS.getTrNo().replace(".0", "") + ", GDN Ref : " + dailyReportNCS.getRefNo(), "Ops tell IT about this Reconciliation Problem : " + e.getMessage());
	}
	
	private void addFailedRecordCausedByErrorMessage(DailyReportNCS dailyReportNCS, ActivityReportData activityReportData, String errorMessage){
		_log.error("Problem " + errorMessage);
		activityReportData.getFailedItemList()
			.put("No : " + dailyReportNCS.getTrNo().replace(".0", "") + ", GDN Ref : " + dailyReportNCS.getRefNo(), errorMessage + " Ops please reupload.");
	}
	
	private LogLogisticsProvider getLogLogisticsProvider(){
		LogLogisticsProvider provider = new LogLogisticsProvider();
		provider.setLogisticsProviderId(1L);
		provider.setLogisticsProviderCode("NCS");
