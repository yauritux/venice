...
//	}
	...
	public boolean createOrder() throws VeniceInternalException {	
		 ...
		 */
		CommonUtil.logDebug(this.getClass().getCanonicalName(), "Mapping the order object to the venOrder object...");
		...
		*/
		...
		CommonUtil.logDebug(this.getClass().getCanonicalName(), "\n done persist order");
			...
			for(String party : merchantProduct){				
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "show venParty in orderItem :  "+party);
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "string merchant :  "+temp[0]+" and "+temp[1]);
				...
				for(int h =0; h< orderItems.size();h++){
							...
							if(venMerchantList.get(0).getVenParty()==null){
								...
								List<VenParty> venPartyList = venPartyDAO.findByLegalName(temp[1]);
									if(venPartyList==null || venPartyList.isEmpty()){
										...
										CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist venParty :  "+venPartyitem.getFullOrLegalName());
										...
										CommonUtil.logDebug(this.getClass().getCanonicalName(), "add  new party for venmerchant (Merchant Id :"
										...
										venMerchantDAO.saveAndFlush(venMerchantList.get(0));
										CommonUtil.logDebug(this.getClass().getCanonicalName(), "add  party for venmerchant (Merchant Id :"
										          + orderItems.get(h).getVenMerchantProduct().getVenMerchant().getWcsMerchantId() +" )"
										);						
									}
								}
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "party is null for venmerchant (Merchant Id :"+ temp[0] +" )");
				}
			...
			if (order.getPayments() != null && !order.getPayments().isEmpty()) {
				...
				while (paymentIterator.hasNext()) {
					...
					if (!next.getPaymentType().equals(VenWCSPaymentTypeConstants.VEN_WCS_PAYMENT_TYPE_PartialFulfillment.desc())) {
						...
						*/
						...
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Mapping the VenOrderPayment object...");
				...
				while (paymentIterator.hasNext()) {
					...
					//if (!next.getVenPaymentType().getPaymentTypeCode().equals(VEN_PAYMENT_TYPE_VA)) {
						...
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Order Amount = "+paymentBalance);
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "paymentBalance.compareTo(new BigDecimal(0)):  "+paymentBalance.compareTo(new BigDecimal(0)) );
						...
						if (paymentBalance.compareTo(new BigDecimal(0)) >= 0) {
							...
							if (paymentBalance.compareTo(paymentAmount) < 0) {
								...
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "Order Allocation Amount is paymentBalance = "+paymentBalance);
							} else {
								allocation.setAllocationAmount(paymentAmount);
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "Order Allocation Amount is paymentAmount = "+paymentAmount);
							}
							...
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "venOrder Payment Allocation List size from looping ke-"
	...
	private List<VenOrderPayment> persistOrderPaymentList(List<VenOrderPayment> venOrderPaymentList) throws VeniceInternalException {
		...
		if (venOrderPaymentList != null && !venOrderPaymentList.isEmpty()) {
			try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrderPayment list...:"+ venOrderPaymentList.size());
				...
				while (i.hasNext()) {
					...
					*/
					...
					if (paymentList.isEmpty()) {
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Payment not found so persisting it...");
						// Persist the object
	...
	private List<VenOrderPaymentAllocation> persistOrderPaymentAllocationList(
		...
		if (venOrderPaymentAllocationList != null	&& !venOrderPaymentAllocationList.isEmpty()) {
			try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrderPaymentAllocation list...:"+ venOrderPaymentAllocationList.size());
				Iterator<VenOrderPaymentAllocation> i = venOrderPaymentAllocationList.iterator();
				while (i.hasNext()) {
					VenOrderPaymentAllocation next = i.next();
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "value of paymentAllocation ......: order_id = "
			...
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrderPaymentAllocation list is null");
		}
	...
	List<VenOrderItem> persistOrderItemList(VenOrder venOrder, List<VenOrderItem> venOrderItemList) throws VeniceInternalException {
		...
		if (venOrderItemList != null && !venOrderItemList.isEmpty()) {
			try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrderItem list...:" + venOrderItemList.size());
				...
				while (i.hasNext()) {
					...
					*/
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "persisting  VenOrderItemAddress" );
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "Total VenOrderItemContactDetail to be persisted => " + venOrderItemContactDetailList.size());
	...
	private Boolean removeOrderPaymentAllocationList(VenOrder venOrder) {
		//try {
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "Remove Order Payment Allocation List ...:order id= "+venOrder.getOrderId()+" and wcs Order Id= "+venOrder.getWcsOrderId());
			/*
			VenOrderPaymentAllocationSessionEJBLocal paymentAllocationHome = (VenOrderPaymentAllocationSessionEJBLocal) this._genericLocator
					.lookupLocal(VenOrderPaymentAllocationSessionEJBLocal.class, "VenOrderPaymentAllocationSessionEJBBeanLocal");
	...
	VenRecipient persistRecipient(VenRecipient venRecipient) throws VeniceInternalException {
		if (venRecipient != null) {
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenRecipient... :" + venRecipient.getVenParty().getFullOrLegalName());
	...
	List<VenOrderItemAdjustment> persistOrderItemAdjustmentList(VenOrderItem venOrderItem, List<VenOrderItemAdjustment> venOrderItemAdjustmentList) 
	   throws VeniceInternalException {
		...
		if (venOrderItemAdjustmentList != null && !venOrderItemAdjustmentList.isEmpty()) {
			try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrderItemAdjustment list...:" + venOrderItemAdjustmentList.size());
				...
				while (i.hasNext()) {
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "id.getOrderItemId: "+id.getOrderItemId());
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "id.getPromotionId: "+id.getPromotionId());
					next.setId(id);
					// Persist the object
					/*
					...
					*/
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "existingVenOrderItemAdjustments.size: "+existingVenOrderItemAdjustments.size());
	...
	private VenOrder persistOrder(Boolean vaPaymentExists, Boolean csPaymentExists, VenOrder venOrder) throws VeniceInternalException {
		CommonUtil.logDebug(this.getClass().getCanonicalName(), "vaPaymentExists: "+vaPaymentExists);
		CommonUtil.logDebug(this.getClass().getCanonicalName(), "csPaymentExists: "+csPaymentExists);
		if (venOrder != null) {
			//try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenOrder... :" + venOrder.getWcsOrderId());				
	...
	private Boolean createOrderStatusHistory(VenOrder venOrder, VenOrderStatus venOrderStatus) 
	  throws VeniceInternalException {
		try {
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "add order status history");
			...
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "done add order status history");
	...
	private VenCustomer persistCustomer(VenCustomer venCustomer) throws VeniceInternalException {
		if (venCustomer != null) {
			try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenCustomer... :" + venCustomer.getCustomerUserName());
				/*
	...
	private Boolean createOrderItemStatusHistory(VenOrderItem venOrderItem, VenOrderStatus venOrderStatus){
		//try {
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "add order item status history");
	...
	List<VenPartyAddress> persistPartyAddresses(List<VenPartyAddress> venPartyAddressList) {
		...
		if (venPartyAddressList != null && !venPartyAddressList.isEmpty()) {
			//try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenPartyAddress list...:" + venPartyAddressList.size());
		...
		if (venParty != null) {
			//try {
				...
				if(type.equals("Customer")){
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenParty... :" + venParty.getVenCustomers().get(0).getCustomerUserName());
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenParty... :" + venParty.getFullOrLegalName());
				...
				if (existingParty != null) {
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "existing Party not null" );
					...
					for(VenPartyAddress venPartyAddress:existingParty.getVenPartyAddresses()){
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "ven Party Address... :"+venPartyAddress.getVenAddress().getAddressId());
					...
					if(venParty.getVenPartyAddresses()!=null){
						for(VenPartyAddress venPartyAddress:venParty.getVenPartyAddresses()){
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "New ven Party Address... :"+venPartyAddress.getVenAddress().getStreetAddress1());
					...
					if(!newAddressList.isEmpty()){
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "New ven Party Address is not empty and Update Address List");
						List<VenAddress> updatedAddressList = this.updateAddressList(existingAddressList, newAddressList);
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "updatedAddressList size => " + updatedAddressList.size());
						List<VenAddress> tempAddressList = new ArrayList<VenAddress>();
						
						tempAddressList.addAll(updatedAddressList);
						
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Remove old VenAddress");
						...
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist Party Addresses ");
						...
						if(updatedAddressList.size() == 0){
							for(VenAddress updatedAddress:tempAddressList){
								...
								for(VenPartyAddress venPartyAddress:venPartyAddressList){
									CommonUtil.logDebug(this.getClass().getCanonicalName(), "VenPartyAddress => " + venPartyAddress.getVenAddress().getAddressId());
								}
							}
						}
						
						//copy existing address list to new list so it can be added new address list
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "copy address list to new list");
//						List<VenPartyAddress> venPartyAddressList2=new ArrayList<VenPartyAddress>(existingParty.getVenPartyAddresses()).subList(0,venPartyAddressList.size());
						...
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "add All VenParty Addresses");
						existingParty.setVenPartyAddresses(venPartyAddressList);
						for(VenPartyAddress venPartyAddress:venPartyAddressList){
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "VenPartyAddress => " + venPartyAddress.getVenAddress().getAddressId());
						}
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "done add All VenParty Addresses");
					}
					...
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "Get old and new party ven contact Detail  ");
					...
					if(venParty.getVenContactDetails()!=null){
							...
							if(!newContactDetailList.isEmpty()){
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "updatedContact Detail List from existingContactDetailList to newContactDetailList");
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "start updating contact detail");
								...
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "done updating contact detail!!!");
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist Address List");
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "synchronize VenParty Reference Data");
				// Synchronize the reference data
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist VenParty ");
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Venpartyaddress size = >" + venParty.getVenPartyAddresses().size());
				...
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist Contact Details");
				// Persist the contact details
				venParty.setVenContactDetails(this.persistContactDetails(venContactDetailList));
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "VenContactDetails size = >" + venParty.getVenContactDetails());
			/*
			} catch (Exception e) {
	...
	List<VenContactDetail> persistContactDetails(List<VenContactDetail> venContactDetailList) throws VeniceInternalException {
		...
		if (venContactDetailList != null && !venContactDetailList.isEmpty()) {
			//try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenContactDetail list...:" + venContactDetailList.size());
	...
	VenAddress persistAddress(VenAddress venAddress) throws VeniceInternalException {
		if (venAddress != null) {
			//try {
				CommonUtil.logDebug(this.getClass().getCanonicalName(), "Persisting VenAddress... :" + venAddress.getStreetAddress1());
				// Synchronize the reference data
				...
				*/
				...
				if (venAddress.getAddressId() == null) {
					if(venAddress.getStreetAddress1()==null && venAddress.getKecamatan()==null && venAddress.getKelurahan()==null && venAddress.getVenCity()==null &&
						...
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "persist address");
						//venAddress = (VenAddress) addressHome.mergeVenAddress(venAddress);
						venAddress = venAddressDAO.saveAndFlush(venAddress);
					}
				} else {
					CommonUtil.logDebug(this.getClass().getCanonicalName(), "merge address");
					//venAddress = (VenAddress) addressHome.mergeVenAddress(venAddress);
					venAddress = venAddressDAO.saveAndFlush(venAddress);
				}
			...
			if (next != null) {
				...
				if (next instanceof VenOrder) {
					if (((VenOrder) next).getWcsOrderId() != null) {
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Restricting VenOrder... :" + ((VenOrder) next).getWcsOrderId());
				...
				if (next instanceof VenOrderPayment) {
					if (((VenOrderPayment) next).getWcsPaymentId() != null) {
						CommonUtil.logDebug(this.getClass().getCanonicalName(), "Synchronizing VenOrderPayment... :" + ((VenOrderPayment) next).getWcsPaymentId());
				...
				if (next instanceof VenParty) {
					if (((VenParty) next).getFullOrLegalName() != null) {
						try {
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "Synchronizing VenParty reference data... ");
				...
				if (next instanceof VenProductType) {
					if (((VenProductType) next).getProductTypeCode() != null) {
						//try {
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "Synchronizing VenProductType... :" + ((VenProductType) next).getProductTypeCode());
							/*
					...
					if (((VenPromotion) next).getPromotionCode() != null) {
						try {
							...
							*/
							...
							CommonUtil.logDebug(this.getClass().getCanonicalName(), "promotionExactList size: "+promotionExactList.size());
														
							if(promotionExactList.size()>0) {
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "exact promo found");
								...
								CommonUtil.logDebug(this.getClass().getCanonicalName(), "exact promo not found, cek uploaded promo");
								/*
	...
	private VenParty retrieveExistingParty(String custUserName) {
		...
		if (customerList != null && !customerList.isEmpty()) {
			...
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "Total existing vencontactdetail => " + venContactDetailList.size());
			...
			CommonUtil.logDebug(this.getClass().getCanonicalName(), "Total existing VenPartyAddress => " + venPartyAddressList.size());
