...
public class VenOrderItemSessionEJBBean implements VenOrderItemSessionEJBRemote,
    ...
    private static final String MERGE_ORDER_ITEM_SQL = "update ven_order_item set etd = ?,  gift_card_flag = ?,  gift_card_note = ?,  "
            + " gift_wrap_price = ?, gift_wrap_flag = ?, insurance_cost = ?, max_est_date = ?, merchant_settlement_flag = ?, "
            + " min_est_date = ?,  package_count = ?,  price = ?,  quantity = ?,  salt_code = ?, shipping_cost = ?,  shipping_weight = ?, "
            + " special_handling_instructions = ?, total = ?, logistics_price_per_kg = ?, shipping_address_id = ?,  product_id = ?, "
            + " order_status_id = ?,  recipient_id = ?,  logistics_service_id = ?, sales_batch_status = ?, transaction_fee_amount = ?, "
            + " cx_date = ?, cx_mta_date = ? "
            + " where order_item_id = ? ";
        ...
        if (totalOrderItem == 0) {
            ...
            try {
                ...
                if (venOrderItem.getTransactionFeeAmount() != null) {
                    ...
                    psMergeOrderItem.setNull(25, Types.NUMERIC);
                }

                if (venOrderItem.getCxDate() != null) {
                    psMergeOrderItem.setTimestamp(26, venOrderItem.getCxDate());
                } else {
                    psMergeOrderItem.setNull(26, Types.TIMESTAMP);
                }
                
                if (venOrderItem.getCxMtaDate() != null) {
                    psMergeOrderItem.setTimestamp(27, venOrderItem.getCxMtaDate());
                } else {
                    psMergeOrderItem.setNull(27, Types.TIMESTAMP);
                }
                psMergeOrderItem.setLong(28, venOrderItem.getOrderItemId());
...
//			}	
    ...
    @Override
    public String getSingleResultUsingNativeQuery(String jpqlStmt) {
        Long startTime = System.currentTimeMillis();
        ...
        int returnList = query.executeUpdate();
