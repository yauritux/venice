...
public class MainPagePresenter extends Presenter<MainPagePresenter.MyView, MainPagePresenter.MyProxy> implements
    ...
    private static String signedInUserRole;
    ...
    /**
     * This method performs RPC call to server to get the user permission and
     * update menu based on the permission
     *
    ...
    /**
     * This method performs RPC call to server to set the user role
     *
     * @param username	the name of user to get role
     */
    private void setUserRole() {
        RPCRequest request = new RPCRequest();

        request.setData("");
        request.setActionURL(GWT.getHostPageBaseURL() + Util.authorizationUtilServlet + "?method=fetchUserRoleList&type=RPC");
        request.setHttpMethod("POST");
        request.setUseSimpleHttp(true);
        request.setShowPrompt(false);
    ...
    /**
     * @return the signedInUserRole
     */
    public static String getSignedInUserRole() {
        return signedInUserRole == null || signedInUserRole.trim().isEmpty() ? "inv_wh_approver;" : signedInUserRole;
    }
    ...
    /**
     * This method performs RPC call to server to get the user permissions and
     * sets the initial user permission on the menus
     *
     * @param pageName	the pageName to update the bread crumb
     */
