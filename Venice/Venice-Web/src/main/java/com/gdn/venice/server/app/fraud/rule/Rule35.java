...
public class Rule35 {
	...
	public int execute(VenOrder venOrder) throws Exception{
		...
		try{
			...
			List<FrdParameterRule35> nameGreyList = sessionHome.queryByRange("select o from FrdParameterRule35 o where upper(o.customerName) = '"+venOrder.getVenCustomer().getVenParty().getFullOrLegalName().toUpperCase().replace("'", "")+"'", 0, 0);
