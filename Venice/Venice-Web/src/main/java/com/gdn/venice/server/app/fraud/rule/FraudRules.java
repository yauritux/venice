...
public class FraudRules {
	...
	public boolean calculateFraudRules(VenOrder venOrder){
		...
		pointRule35=0, pointRule36=0,pointRule37=0, pointRule38=0, pointRule39=0, pointRule40=0, pointRule41=0, pointRule42=0, pointRule43=0, pointRule44=0;
		...
		try{
			...
			//attempt rule
			 Rule41 rule41 = new Rule41();
			 pointRule41=rule41.execute(venOrder.getWcsOrderId());
			 ...
			 Rule44 rule44 = new Rule44();
			 pointRule44=rule44.execute(venOrder.getOrderId());
			...
			pointRule42+pointRule43+pointRule44;
			...
			try {
				...
				if(fraudCaseList==null || fraudCaseList.size()==0){	
					try{
						...
						isSuccessCalculate=false;
						_log.error("Fraud calculate failed when persisting fraud points or order id is already exist for wcs order id: "+venOrder.getWcsOrderId());
						e.printStackTrace();
						...
						fraudPoint.setFrdFraudSuspicionCase(fraudCase);
						fraudPoint.setFraudRuleName(DataConstantNameTokens.FRAUD_RULE_41);
						...
						fraudPoint.setFrdFraudSuspicionCase(fraudCase);
						fraudPoint.setFraudRuleName(DataConstantNameTokens.FRAUD_RULE_44);
