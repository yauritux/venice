	...
	public RafDsResponse execute() {
		...
		try {
			...
			if(finJournalTransactionList!=null && !finJournalTransactionList.isEmpty()){
				...
				String select = "select o from FinApPayment o where o.finApprovalStatus.approvalStatusId <> "+VeniceConstants.FIN_APPROVAL_STATUS_DONE;				
				finApPaymentList = sessionHome.queryByRange(select, 0, 0);
			} else {
				...
				if (criteriaAndTaskCriteria!=null) {
					...
					JPQLAdvancedQueryCriteria criteriaStatusDone = new JPQLAdvancedQueryCriteria("and");
					JPQLSimpleQueryCriteria simpleCriteria = new JPQLSimpleQueryCriteria();
					simpleCriteria.setFieldName(DataNameTokens.FINAPPAYMENT_FINAPPROVALSTATUS_APPROVALSTATUSID);
					simpleCriteria.setOperator("notEquals");
					simpleCriteria.setValue(VeniceConstants.FIN_APPROVAL_STATUS_DONE+"");
					simpleCriteria.setFieldClass(DataNameTokens.getDataNameToken().getFieldClass(DataNameTokens.FINAPPAYMENT_FINAPPROVALSTATUS_APPROVALSTATUSID));
					criteriaStatusDone.add(criteria);
					criteriaStatusDone.add(simpleCriteria);										
			...
			for (int i=0;i<finApPaymentList.size();i++) {
				...
				map.put(DataNameTokens.FINAPPAYMENT_APPAYMENTDATE, (finApPayment.getApPaymentDate()!=null)?formatter.format(finApPayment.getApPaymentDate()):"");
