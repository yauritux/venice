...
//		refundPaymentDetailListGrid.getField(DataNameTokens.FINARFUNDSINREFUND_FINARFUNDSINRECONRECORD_ORDERDATE).setDateFormatter(DateDisplayFormat.TOEUROPEANSHORTDATETIME);
		...
		refundPaymentDetailListGrid.addSelectionChangedHandler(new SelectionChangedHandler() {
			...
			@Override
			public void onSelectionChanged(SelectionEvent event) {
				String action ="";	boolean sameActionOrNot =false;
				for(ListGridRecord list : refundPaymentDetailListGrid.getSelection()){
					if(action.equals("")){
						action=list.getAttribute(DataNameTokens.FINARFUNDSINREFUND_ACTION_TAKEN);
					}else	if(!action.equals(list.getAttribute(DataNameTokens.FINARFUNDSINREFUND_ACTION_TAKEN))){
						sameActionOrNot=true;
						break;
					}					
				}
	...
	public void loadPaymentDataForPaymentProcessing(DataSource dataSource, String paymentData) {
		if (paymentData.equals("MERCHANT")) {
			...
			makePaymentButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					for (int i=0;i<refundPaymentRecords.length;i++) {
						...
						refundPaymentMap.put("FEEAMOUNT", refundPaymentRecords[i].getAttributeAsString(DataNameTokens.FINARFUNDSINREFUND_FEEAMOUNT));
			...
			refundPaymentListGrid.addEditCompleteHandler(new EditCompleteHandler(){

				@Override
				public void onEditComplete(EditCompleteEvent event) {
					ListGridRecord[] refundPaymentRecords = refundPaymentListGrid.getSelection();
					String cekId="";
					boolean bExistsRecordWithNotSameBankAccountId = false;
					for (int i=0;i<refundPaymentRecords.length;i++) {
							if(cekId.equals("")) cekId=refundPaymentRecords[i].getAttributeAsString(DataNameTokens.FINAPPAYMENT_FINACCOUNT_ACCOUNTID);
							if (!cekId.equals(refundPaymentRecords[i].getAttributeAsString(DataNameTokens.FINAPPAYMENT_FINACCOUNT_ACCOUNTID) )) {
								bExistsRecordWithNotSameBankAccountId = true;
								break;
							}
					}					
					makePaymentButton.setDisabled(bExistsRecordWithNotSameBankAccountId);					
				}				
			});
			...
			refundPaymentListGrid.addSelectionChangedHandler(new SelectionChangedHandler() {				
				...
				public void onSelectionChanged(SelectionEvent event) {
					...
					makePaymentButton.setDisabled(refundPaymentListGrid.getSelection().length==0 || bExistsRecordWithNotSameBankAccountId);
				}
			});
