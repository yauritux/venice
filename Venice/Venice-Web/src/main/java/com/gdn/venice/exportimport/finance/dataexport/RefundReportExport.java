		...
		if (!finArFundsInRefundList.isEmpty()){					
			...
			for (FinArFundsInRefund item : finArFundsInRefundList){
					HSSFRow row = sheet.createRow(++startRow);		
			    	 ...
			    	 nameCell = row.createCell(startCol+10);nameCell.setCellValue(new HSSFRichTextString(item.getFinApPayment()!=null?(item.getFinApPayment().getApPaymentDate()!=null? dateFormat.format(item.getFinApPayment().getApPaymentDate()):null):null));	 	 
