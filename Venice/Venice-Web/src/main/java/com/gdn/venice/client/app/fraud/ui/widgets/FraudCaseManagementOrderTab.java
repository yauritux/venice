...
public class FraudCaseManagementOrderTab extends Tab {
	...
	ListGrid attempPaymentListGrid = null;
	
	public FraudCaseManagementOrderTab(String title, DataSource orderDetailData, DataSource orderItemData, DataSource MIGSMasterUpload) {
		super(title);		
...
//		orderDetailForm.getField(DataNameTokens.VENORDER_ORDERDATE).setDateFormatter(DateDisplayFormat.TOEUROPEANSHORTDATE);
		...
		//Grid attemp payment
		VLayout fraudCaseAttempPaymentLayout  = new VLayout();
		Label fraudCaseAttempPaymentLabel = new Label("<b>Attemp Payment:</b>");
		fraudCaseAttempPaymentLabel.setHeight(10);


		attempPaymentListGrid = new ListGrid();
		attempPaymentListGrid.setShowRollOver(false);
		attempPaymentListGrid.setWidth100();
		attempPaymentListGrid.setHeight100();
		attempPaymentListGrid.setSortField(0);
		attempPaymentListGrid.setShowRowNumbers(true);
		attempPaymentListGrid.setAutoFetchData(true);			
		attempPaymentListGrid.setDataSource(MIGSMasterUpload);
		attempPaymentListGrid.setFields(
			new ListGridField(DataNameTokens.MIGSMASTER_MERCHANTTRANSACTIONREFERENCE, 200),
			new ListGridField(DataNameTokens.MIGSMASTER_RESPONSECODE,200),
			new ListGridField(DataNameTokens.MIGSMASTER_DIALECTCSCRESULTCODE, 200),
			new ListGridField(DataNameTokens.MIGSMASTER_CARDNUMBER, 200),
			new ListGridField(DataNameTokens.MIGSMASTER_VENORDERPAYMENT_THREEDSSECURITYLEVELAUTH, 200)
		);
		
		
		//Put all components into layout
		fraudCaseAttempPaymentLayout.setMembers(fraudCaseAttempPaymentLabel,attempPaymentListGrid);
		orderLayout.setMembers(orderDetailLayout, fraudCaseOrderItemLayout,fraudCaseAttempPaymentLayout);
		setPane(orderLayout);
	}
