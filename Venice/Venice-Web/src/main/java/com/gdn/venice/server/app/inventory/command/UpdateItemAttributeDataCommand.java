 ...
 * @author Roland
 */
public class UpdateItemAttributeDataCommand implements RafDsCommand {

    private RafDsRequest request;
    GRNManagementService grnService;
    protected static Logger _log = null;
    
    public UpdateItemAttributeDataCommand(RafDsRequest request) {
        this.request = request;
        Log4jLoggerFactory loggerFactory = new Log4jLoggerFactory();
        _log = loggerFactory.getLog4JLogger("com.gdn.venice.server.app.inventory.command.UpdateItemAttributeDataCommand");
    }

    @Override
    public RafDsResponse execute() {
    	_log.info("UpdateItemAttributeDataCommand");
        ...
        try {
			...
			for(int i=0;i< dataList.size();i++){
				...
				while(iter.hasNext()){
					...
					}else if(key.equals(DataNameTokens.INV_ITEM_ATTRIBUTE_NAME)){
