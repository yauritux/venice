...
public class PackingListView extends ViewWithUiHandlers<PackingListUiHandler> implements
    ...
    private Window buildPackingDetailWindow(final ListGridRecord record) {
        ...
        final String pickPackageId = record.getAttribute(DataNameTokens.INV_PACKING_PICKPACKAGE_ID);
        ...
        TextItem pickPackageCode = new TextItem(DataNameTokens.INV_PACKING_PICKPACKAGE_CODE, "Pick Package Code");
        pickPackageCode.setValue(record.getAttribute(DataNameTokens.INV_PACKING_PICKPACKAGE_CODE));

        TextItem containerId = new TextItem(DataNameTokens.INV_PACKING_PICKPACKAGE_CONTAINERID, "Container ID");
        containerId.setValue(record.getAttribute(DataNameTokens.INV_PACKING_PICKPACKAGE_CONTAINERID));

        TextItem awbNumber = new TextItem(DataNameTokens.INV_PACKING_AWB_NUMBER, "AWB Number");
        awbNumber.setValue(record.getAttribute(DataNameTokens.INV_PACKING_AWB_NUMBER));

        TextItem claimedBy = new TextItem(DataNameTokens.INV_PACKING_PICKPACKAGE_CLAIMEDBY, "Claimed By");
        claimedBy.setValue(record.getAttribute((DataNameTokens.INV_PACKING_PICKPACKAGE_CLAIMEDBY)));

        packingInfoForm.setFields(packingNo, pickPackageCode, containerId, awbNumber, claimedBy);
        ...
        DataSource ds = PackingListData.getAllSalesData(pickPackageId, username);
        ...
        if (claimer == null || "null".equals(claimer) || "".equals(claimer)
                || (claimer.equals(username.trim())
                && !record.getAttribute(DataNameTokens.INV_PACKING_PICKPACKAGE_STATUS).equalsIgnoreCase("complete"))) {
        ...
        submitButton.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                getUiHandlers().onSavePacking(MainPagePresenter.signedInUser, pickPackageId, record.getAttribute(DataNameTokens.INV_PACKING_AWB_NUMBER));
            }
        });
    ...
    public void loadPackingData(String warehouseId) {
        ...
        ds.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_STATUS).setValueMap(valueMap);

        ListGridField listGridField[] = Util.getListGridFieldsFromDataSource(ds);

        packingListGrid.setDataSource(ds);
        packingListGrid.setFields(listGridField);
        packingListGrid.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_ID).setHidden(Boolean.TRUE);
        packingListGrid.getField(DataNameTokens.INV_PACKING_AWB_LOGISTIC).setCanFilter(true);
        packingListGrid.getField(DataNameTokens.INV_PACKING_AWB_NUMBER).setCanFilter(true);
        packingListGrid.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_CLAIMEDBY).setCanFilter(true);
        packingListGrid.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_CODE).setCanFilter(true);
        packingListGrid.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_HANDLING).setCanFilter(true);
        packingListGrid.getField(DataNameTokens.INV_PACKING_PICKPACKAGE_PICKER).setCanFilter(true);
