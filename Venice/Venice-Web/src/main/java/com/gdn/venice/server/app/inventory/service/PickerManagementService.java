 ...
 * @author Maria Olivia
 */
public class PickerManagementService {

    HttpClient httpClient;
    ObjectMapper mapper;

    public PickerManagementService() {
        httpClient = new HttpClient();
        mapper = new ObjectMapper();
    }

    public InventoryPagingWrapper<Picker> getPickerData(RafDsRequest request) throws HttpException, IOException {
    ...
    public ResultWrapper<Picker> nonActivePicker(String username, String id) throws HttpException, IOException {
        String url = InventoryUtil.getStockholmProperties().getProperty("address")
                + "picker/nonActive?username=" + username + "&id=" + id;
        System.out.println(url);
        ...
        if (httpCode == HttpStatus.SC_OK) {
            ...
            is.close();
            System.out.println(sb.toString());
            return mapper.readValue(sb.toString(), new TypeReference<ResultWrapper<Picker>>() {
            });
        } else {
            return null;
        }
    ...
    public ResultWrapper<Picker> saveOrUpdatePicker(String username, Picker picker) throws JsonGenerationException, JsonMappingException, IOException {
        String url = InventoryUtil.getStockholmProperties().getProperty("address")
                + "picker/saveOrUpdate?username=" + username;
        System.out.println(url);
        PostMethod httpPost = new PostMethod(url);
        String json = mapper.writeValueAsString(picker);
        System.out.println(json);
        httpPost.setRequestEntity(new ByteArrayRequestEntity(json.getBytes(), "application/json"));
        httpPost.setRequestHeader("Content-Type", "application/json");
