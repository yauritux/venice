...
public class FetchFundInReconciliationDataCommand implements RafDsCommand {
	...
	public RafDsResponse execute() {
		...
		String customerIdCriteriaValue=null;
		...
		if (criteria!=null) {
			...
			for (int i=0;i<simpleCriteriaList.size();i++) {
				if (simpleCriteriaList.get(i).getFieldName().equals(DataNameTokens.FINARFUNDSINRECONRECORD_AGING)) {
					...
					criteria.remove(simpleCriteriaList.get(i));
				}else if(simpleCriteriaList.get(i).getFieldName().equals(DataNameTokens.FINARFUNDSINRECONRECORD_VENORDERPAYMENT_VENORDERPAYMENTALLOCATION_VENORDER_VENCUSTOMER_CUSTOMERUSERNAME)){
					customerIdCriteriaValue=simpleCriteriaList.get(i).getValue();
					criteria.remove(simpleCriteriaList.get(i));
				}
			}
		}
		...
		try {
			...
			if (journalGroupId!=null) {
				...
				fundInReconRecordList = sessionHome.queryByRange(select, 0, 0);				
				 ...
				 if(customerIdCriteriaValue!=null){						
					List<FinArFundsInReconRecord> fundInReconRecordCriteriaList =null;
					if(criteria!=null && criteria.getListIterator().hasNext()){
						...
						fundInReconRecordCriteriaList = sessionHome.findByFinArFundsInReconRecordLike(finArFundsInReconRecord, criteria, 0, 0);							
					}
					if(fundInReconRecordCriteriaList!=null && !fundInReconRecordCriteriaList.isEmpty()){
						String fundsInReconRecordId="";
						for(FinArFundsInReconRecord item : fundInReconRecordCriteriaList){
							if(fundsInReconRecordId.equals("")) fundsInReconRecordId=item.getReconciliationRecordId()+"";
							else fundsInReconRecordId=fundsInReconRecordId+","+item.getReconciliationRecordId();
						}
						String sql = "select o from FinArFundsInReconRecord o  " +
							"where o.wcsOrderId in (select u.wcsOrderId from VenOrder u where u.venCustomer.customerUserName = '"+customerIdCriteriaValue+"') and  o.reconciliationRecordId IN ("+fundsInReconRecordId+") and o.finArFundsInActionApplied.actionAppliedId <> "+VeniceConstants.FIN_AR_FUNDS_IN_ACTION_APPLIED_REMOVED;
						fundInReconRecordList = sessionHome.queryByRange(sql, 0, 0);	
					}else{
						String sql = "select o from FinArFundsInReconRecord o  " +
							"where o.wcsOrderId in (select u.wcsOrderId from VenOrder u where u.venCustomer.customerUserName = '"+customerIdCriteriaValue+"') and o.finArFundsInActionApplied.actionAppliedId <> "+VeniceConstants.FIN_AR_FUNDS_IN_ACTION_APPLIED_REMOVED;
						fundInReconRecordList = sessionHome.queryByRange(sql, 0, 0);
					}						
				}else if ((criteria == null && criteriaAndTaskCriteria == null) 
				...
				if(fundInReconRecord.getWcsOrderId()!=null){
					...
					if(venOrderList!=null && venOrderList.size() > 0 ) {
						...
						if(show){
							...
							map.put(DataNameTokens.FINARFUNDSINRECONRECORD_VENORDERPAYMENT_VENORDERPAYMENTALLOCATION_VENORDER_VENCUSTOMER_CUSTOMERUSERNAME, venOrderList.get(0).getVenCustomer()!=null?venOrderList.get(0).getVenCustomer().getCustomerUserName():"");
						}			
					}
				}else{
