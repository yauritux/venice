...
/**
 * This class provides a data source request container for general use along
 * with conversion utility methods to convert responses to and from SmartClient
 * XML responses.
 *
...
public class RafDsRequest {
    ...
    public static RafDsRequest convertXmltoRafDsRequest(String xmlContent)
        ...
        if (rafDsRequest.getOperationType().equals("fetch")) {
            ...
            rafDsRequest.setData(getModifiedValuesData(xmlContent));
            rafDsRequest.setOldValues(getOldValuesData(xmlContent));
        }
    ...
    /*
     * Do some refactoring by Maria Olivia 20140414
     */
    public static List<HashMap<String, String>> getModifiedValuesData(String xmlContent) throws Exception {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        DocumentBuilder builder = factory.newDocumentBuilder();
        Document doc = builder.parse(new InputSource(new StringReader(
                xmlContent)));
                    ...
                    for (int j = 0; j < dataFieldNodes.getLength(); j++) {
                        if (dataFieldNodes.item(j) instanceof Element) {
                            ...
                            if (DataNameTokens.getDataNameToken().getFieldClass(dataKey) != null) {
                                ...
                                data.put(dataKey, dataFieldNodes.item(j).getTextContent());
                            }
     ...
     * Do some refactoring by Maria Olivia 20140414
     */
    public static List<HashMap<String, String>> getOldValuesData(String xmlContent) throws Exception {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        DocumentBuilder builder = factory.newDocumentBuilder();
        Document doc = builder.parse(new InputSource(new StringReader(
                xmlContent)));
        ...
        if (oldValueNodeList != null && oldValueNodeList.getLength() > 0) {
             ...
             */
            List<HashMap<String, String>> oldValuesList = new ArrayList<HashMap<String, String>>();
            for (int i = 0; i < oldValueNodeList.getLength(); i++) {
                ...
                for (int j = 0; j < oldValueList.getLength(); j++) {
                    ...
                    if (DataNameTokens.getDataNameToken()
                        ...
                        oldList.put(key, entry.getTextContent());
                    }
                }
    ...
    /**
     * This inner class provides a container for the SmartCLient criteria for
     * the request
     */
    ...
    /**
     * This method determines which types should use queals as the operator
     * instaead of iContains
     *
     * @param fieldClass
