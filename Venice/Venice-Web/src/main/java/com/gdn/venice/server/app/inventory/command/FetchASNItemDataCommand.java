...
public class FetchASNItemDataCommand implements RafDsCommand {
    ...
    /*
     * Edited by Maria Olivia - 20140429
     */
    @Override
            ...
            if (asnWrapper != null && asnWrapper.isSuccess()) {
                for (AdvanceShipNoticeItem asnItem : asnWrapper.getContent()) {
                    if (asnItem.getAdvanceShipNotice().getReferenceType().name().equals(ASNReferenceType.PURCHASE_ORDER.name())) {
                        ...
                        if (poItemWrapper != null && poItemWrapper.isSuccess() && poItemWrapper.getContent() != null) {
                            ...
                            map.put(DataNameTokens.INV_POCFF_QTY, asnItem.getQuantity() + "");
                            map.put(DataNameTokens.INV_POCFF_ITEMUNIT, item.getItem().getItemUnit());
                            map.put(DataNameTokens.INV_POCFF_ITEMLENGTH, item.getItem().getLength() + "");
                            map.put(DataNameTokens.INV_POCFF_ITEMWIDTH, item.getItem().getWidth() + "");
                            map.put(DataNameTokens.INV_POCFF_ITEMHEIGHT, item.getItem().getHeight() + "");
                            map.put(DataNameTokens.INV_POCFF_VOLUME, (item.getItem().getLength() * item.getItem().getWidth() * item.getItem().getHeight()) + "");
                            map.put(DataNameTokens.INV_POCFF_ITEMWEIGHT, item.getItem().getWeight() + "");
                            map.put(DataNameTokens.INV_POCFF_QTYGRN, asnItem.getGrnQuantity() + "");
                            map.put(DataNameTokens.INV_POCFF_ITEMID, item.getItem().getId() + "");
                        ...
                        if (cffItemWrapper != null && cffItemWrapper.isSuccess() && cffItemWrapper.getContent() != null) {
