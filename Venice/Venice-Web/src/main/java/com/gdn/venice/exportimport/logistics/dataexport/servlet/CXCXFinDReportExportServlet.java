...
public class CXCXFinDReportExportServlet extends HttpServlet {
	...
	private static final String EXPORT_REPORT_CX_SQL = "select order_item_id from ven_order_item_status_history " +
													   "where order_status_id="+VeniceConstants.VEN_ORDER_STATUS_CX+" and status_change_reason not like '%CX Finance%' and status_change_reason not like '%CX MTA%' and history_timestamp>to_timestamp(?,'MM/DD/YYYY') and history_timestamp<to_timestamp(?,'MM/DD/YYYY')";
	
	private static final String EXPORT_REPORT_CX_AIRWAYBILL_LIST_SQL = "select airway_bill_number, gdn_reference, wcs_order_id, wcs_order_item_id, h.history_timestamp from log_airway_bill ab " +
																		 "inner join ven_order_item oi on ab.order_item_id = oi.order_item_id " +
																		 "inner join ven_order o on oi.order_id = o.order_id " +
																		 "inner join ven_order_item_status_history h on h.order_item_id=oi.order_item_id "+
																		 "where h.order_status_id="+VeniceConstants.VEN_ORDER_STATUS_CX+" and status_change_reason not like '%CX Finance%' and status_change_reason not like '%CX MTA%' " +
																		 "and ab.order_item_id=?";
	
	private static final String EXPORT_REPORT_CX_FINANCE_SQL = "select order_item_id from ven_order_item_status_history " +
															   "where order_status_id="+VeniceConstants.VEN_ORDER_STATUS_CX+" and status_change_reason like '%CX Finance%' and history_timestamp>to_timestamp(?,'MM/DD/YYYY') and history_timestamp<to_timestamp(?,'MM/DD/YYYY')";
	
	private static final String EXPORT_REPORT_CX_FINANCE_AIRWAYBILL_LIST_SQL = "select full_or_legal_name, wcs_order_id, wcs_order_item_id, order_date, h.history_timestamp from log_airway_bill ab " +
																				 "inner join ven_order_item oi on ab.order_item_id = oi.order_item_id " +
																				 "inner join ven_order o on oi.order_id = o.order_id " +
																				 ...
																				 "inner join ven_order_item_status_history h on h.order_item_id=oi.order_item_id "+
																				 "where h.order_status_id="+VeniceConstants.VEN_ORDER_STATUS_CX+" and status_change_reason like '%CX Finance%' " +
																				 "and ab.order_item_id=?";
	 
	private static final String EXPORT_REPORT_D_SQL = "select order_item_id from ven_order_item_status_history " +
										 				"where order_status_id="+VeniceConstants.VEN_ORDER_STATUS_D+" and history_timestamp>to_timestamp(?,'MM/DD/YYYY') and history_timestamp<to_timestamp(?,'MM/DD/YYYY')";

	private static final String EXPORT_REPORT_D_AIRWAYBILL_LIST_SQL = "select gdn_reference, wcs_order_id, wcs_order_item_id, received, recipient, relation, h.history_timestamp from log_airway_bill ab " +
																	   "inner join ven_order_item oi on ab.order_item_id = oi.order_item_id " +
																	   "inner join ven_order o on oi.order_id = o.order_id " +
																	   "inner join ven_order_item_status_history h on h.order_item_id=oi.order_item_id "+
																	   "where h.order_status_id="+VeniceConstants.VEN_ORDER_STATUS_D+
																	   " and ab.order_item_id=?";
	/**
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		...
		String sToDate = request.getParameter("dateTo");
		
		_log.debug("type: "+type);
		_log.debug("dateFrom: "+sFromDate);
		_log.debug("dateTo: "+sFromDate);
		
		String dateForFileNameString = sFromDate.replace("/", "")+"-"+sToDate.replace("/", "");
		...
		if(type.equals("reportCX")){
			...
			try {
				...
				psExportCX.setString(2, sToDate);	
						...
						if(totalAirwaybillList>0){
							...
							if (!awb.getAirwaybillNumber().isEmpty()) {	
								...
								while (rsAirwaybillList.next()) {		
									...
									awb.setCxDate(rsAirwaybillList.getString("history_timestamp"));
			...
			try {
				...
				psExportCXFinance.setString(2, sToDate);	
						...
						while (rsExportCXFinance.next()) {
							...
							if(totalAirwaybillList>0){					
								...
								while (rsAirwaybillList.next()) {		
									...
									awb.setCxFinanceDate(rsAirwaybillList.getString("history_timestamp"));
									airwayBillDataList.add(awb);
								}
							}
						}				
					...
					while (rsExportD.next()) {
						...
						if(totalAirwaybillList>0){
							...
							while (rsAirwaybillList.next()) {		
								...
								awb.setDDate(rsAirwaybillList.getString("history_timestamp"));
								airwayBillDataList.add(awb);
							}
						}
					}				
