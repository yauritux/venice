...
import com.gdn.venice.server.app.fraud.presenter.commands.FetchMIGSUploadMaster;
import com.gdn.venice.server.app.fraud.presenter.commands.FetchOrderCalculateFraud;
import com.gdn.venice.server.app.fraud.presenter.commands.FetchOrderHistory;
...
public class FraudCaseMaintenancePresenterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (type.equals(RafDsCommand.DataSource)) {
			...
			}else if (method.equals("fetchOrderHistory")) {
				RafDsCommand fetchOrderHistory = new FetchOrderHistory(rafDsRequest);
				RafDsResponse rafDsResponse = fetchOrderHistory.execute();
				try {
					retVal = RafDsResponse.convertRafDsResponsetoXml(rafDsResponse);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}else if (method.equals("fetchMIGSUploadMaster")) {
				HashMap<String, String> params = new HashMap<String, String>();				
				params.put(DataNameTokens.FRDFRAUDSUSPICIONCASE_VENORDER_WCSORDERID, request.getParameter(DataNameTokens.FRDFRAUDSUSPICIONCASE_VENORDER_WCSORDERID));
				rafDsRequest.setParams(params);
				RafDsCommand fetchMIGSUploadMaster = new FetchMIGSUploadMaster(rafDsRequest);
				RafDsResponse rafDsResponse = fetchMIGSUploadMaster.execute();
				try {
					retVal = RafDsResponse.convertRafDsResponsetoXml(rafDsResponse);
				} catch (Exception e) {
					e.printStackTrace();
